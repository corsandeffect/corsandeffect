# Hi, I'm CORS and Effect 👋

I build lightweight, functional browser extensions designed to solve everyday workflow problems and improve productivity. 

---

## 🛠️ My Focus
* **Platforms**: Google Chrome, Mozilla Firefox, Microsoft Edge
* **Core Tech**: JavaScript, WebExtensions API, HTML5, CSS3
* **Design Philosophy**: Minimalist UI, zero bloat, and maximum privacy.

---

## 🚀 Featured Extension
* **YourKey AI: OpenAI, Claude and Gemini Side Panel** - A secure, local-storage AI side panel with direct-to-API connections powered by your own personal OpenAI, Claude, and Gemini keys. 
  * 🌐 **Listing Link**: [YourKey AI Side Panel](https://chromewebstore.google.com/detail/yourkey-ai-openai-claude/dlpopklnmgnpiefajcbfioedeeebfkmg)
  * 🛠️ **Status**: Fully functional and launched on Chrome Web Store.

---

## 📬 Contact & Support
If you need support regarding an extension, feel free to reach out:
* **Email**: `corsandeffect@gmail.com`
* **Product Hunt**: [CORS and Effect](https://www.producthunt.com/@corsandeffect)
* **SideProjectors**: [CORS and Effect](https://www.sideprojectors.com/user/profile/247960/) 

---
